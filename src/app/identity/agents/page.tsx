...
import { MOCK_AGENTS, searchAgents } from '@/lib/identity-mock';
...
function AgentsPageInner() {
  ...
  const filteredAgents = useMemo(() => {
    let agents = query ? searchAgents(query) : [...MOCK_AGENTS];
  ...
  const filters = [
    ...
    { key: 'twitter', label: t('sid.agents.filterTwitter') },
              ...
              <tbody>
                {paginatedAgents.map(agent => (
                  <tr key={agent.agentId} className="border-b border-[#2B3139]/50 hover:bg-[#1E2329]/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className="text-xs text-[#848E9C] font-mono">{truncAddr(agent.owner)}</span>
                    </td>
                    <td className="px-4 py-3">
