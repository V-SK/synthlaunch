...
import { getAgentById } from '@/lib/identity-mock';
...
export default function AgentDetailPage() {
  ...
  const dateStr = new Date(agent.createdAt * 1000).toLocaleString();
  const platformLink = PLATFORM_LINKS[agent.platform]?.(agent.platformId) || '#';

  // Mock metadata for ERC-8004 display
  const metadata = [
    { key: 'agent.name', value: agent.name },
    { key: 'agent.platform', value: agent.platform },
    { key: 'agent.platformId', value: agent.platformId },
    { key: 'agent.description', value: agent.description },
    { key: 'agent.skills', value: JSON.stringify(agent.skills) },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <BnbCard className="p-6">
              <h2 className="text-base font-bold text-[#EAECEF] mb-1">{t('sid.detail.metadata')}</h2>
              <p className="text-xs text-[#848E9C] mb-4">{t('sid.detail.metadataDesc')}</p>
              <div className="space-y-2">
                {metadata.map((m, i) => (
                  <div key={i} className="flex items-start gap-3 py-2 border-b border-[#2B3139]/50 last:border-0">
                    <span className="text-xs text-[#F0B90B] font-mono min-w-[160px] flex-shrink-0">{m.key}</span>
                    <span className="text-xs text-[#EAECEF] font-mono break-all">{m.value}</span>
                  </div>
                ))}
              </div>
            </BnbCard>
          ...
          <div className="space-y-6">
            <BnbCard className="p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-sm text-[#EAECEF] font-mono mt-0.5">{agent.agentId}</p>
                </div>
                <div>
                  <span className="text-[10px] text-[#848E9C] uppercase tracking-wider">{t('sid.detail.owner')}</span>
                  <p className="text-xs text-[#EAECEF] font-mono mt-0.5 break-all">{agent.owner}</p>
                </div>
                <div>
            ...
            {/* BscScan link */}
            <a
              href={`https://bscscan.com/token/${SYNTHID_ADDRESS}?a=${agent.agentId}`}
              target="_blank"
              rel="noopener noreferrer"
              className="block"
            >
                ...
                <BnbCard hover className="p-4 flex items-center justify-between group cursor-pointer">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-[#F0B90B]/10 flex items-center justify-center text-sm">
                      {agent.platform === 'moltbook' ? '🦞' : '𝕏'}
                    </div>
                    <span className="text-sm text-[#EAECEF] group-hover:text-[#F0B90B] transition-colors">
                      View on {agent.platform === 'moltbook' ? 'Moltbook' : 'Twitter'}
                    </span>
                  </div>
                  <span className="text-[#848E9C] group-hover:text-[#F0B90B] transition-colors">↗</span>
                </BnbCard>
